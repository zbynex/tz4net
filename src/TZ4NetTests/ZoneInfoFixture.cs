...
namespace TZ4NetTests
{
	...
	public class ZoneInfoFixture
	{
		...
		{
			ZoneInfo zi = new ZoneInfo("UTC");
			long t1 = zi.GetClockFromLocal(ZoneInfo.Time.MaxValue);
			Assert.IsTrue(t1 == ZoneInfo.MaxClock);
			long t2 = zi.GetClockFromLocal(ZoneInfo.Time.MaxValue);
		...
		{
			ZoneInfo.Time time = new ZoneInfo.Time(2038, 01, 19, 03, 14, 08);
			ZoneInfo zi = new ZoneInfo("UTC");
			long t = zi.GetClockFromLocal(time);
		}
		...
		{
			ZoneInfo zi = new ZoneInfo("UTC");
			long t1 = zi.GetClockFromLocal(ZoneInfo.Time.MinValue);
		...
		{
			ZoneInfo.Time time = new ZoneInfo.Time(1901, 12, 13, 20, 45, 51);
			ZoneInfo zi = new ZoneInfo("UTC");
			long t = zi.GetClockFromLocal(time);
		}
		...
		{
			ZoneInfo zi = new ZoneInfo("Europe/Warsaw");
			long t = zi.GetClockFromLocal(new ZoneInfo.Time(2006, 10, 29, 01, 59, 59));
			Assert.IsTrue(t == 1162079999);
			t = zi.GetClockFromLocal(new ZoneInfo.Time(2006, 10, 29, 02, 12, 29));
			Assert.IsTrue(t == 1162080749 || t == 1162084349);
			t = zi.GetClockFromLocal(new ZoneInfo.Time(2006, 10, 29, 02, 12, 30));
			Assert.IsTrue(t == 1162080750 || t == 1162084350);
			t = zi.GetClockFromLocal(new ZoneInfo.Time(2006, 10, 29, 02, 12, 31));
			Assert.IsTrue(t == 1162080751 || t == 1162084351);
			t = zi.GetClockFromLocal(new ZoneInfo.Time(2006, 10, 29, 02, 12, 32));
