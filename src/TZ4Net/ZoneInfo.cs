...
namespace TZ4Net
{
	...
	public class ZoneInfo
	{
		...
		public class Time
		{
			...
			internal Time(long year, int mon, int day, int secs) 
			{
				this.year = year;
				this.mon = mon;
				this.mday = day;
			...
			/// <returns></returns>
			public Time(long year, int mon, int day, int hour, int min, int sec) 
			...
			public override string ToString()
            {
                ...
                The calculations (...) are valid for the whole range of dates since -4800. For dates before 1582, 
                the resulting date components are valid only in the Gregorian proleptic calendar.
                For dates in that era (before year AD 1), astronomical year numbering is used. 
                This includes a year zero, which immediately precedes AD 1. 
                Astronomical year zero is 1 BC in the proleptic Gregorian calendar and, in general, 
                proleptic Gregorian year (n BC) = astronomical year (Y = 1 - n). 
                For astronomical year Y (Y < 1), the proleptic Gregorian year is (1 - Y) BC.
                 */

                string era = "AD";
                long eraYear = year + 1900;
                if (eraYear <= 0)
                {
                    era = "BC";
                    eraYear = 1 - eraYear;
                ...
                return string.Format("{0}-{1:00}-{2:00} {3} {4:00}:{5:00}:{6:00} {7}", eraYear, mon + 1, mday, era, hour, min, sec, zone);
            }
			...
			public DateTime DateTime 
			{
				get 
				{
                    ...
                    try
                    {
                        ret = new DateTime((int)Year, Mon, MDay, Hour, Min, second).AddSeconds(leapSecond);
                    }
                    catch (ArgumentException e)
                    {
                        throw new InvalidOperationException(string.Format("Time {0} is not in valid DateTime range.", this), e);
                    }
                    return ret;
				}
			}
			...
			{
				return (year.GetHashCode() << 24) + (mon << 20) + (mday << 15) + (hour << 10) + (min << 5) + sec;
			}

            /// <summary>
            /// Sets the local time fields from a clock and GMT offset.
            /// </summary>
            ...
            {
                long x = clock / SECSPERDAY;
                long days = clock / SECSPERDAY;
                ...
                // Gregorian calendar from Julian day number conversion. See http://en.wikipedia.org/wiki/Julian_day
                long J = days + 2440588;                    // JD 2440588 corresponds to 1 Jan, 1970.
                long j = J + 32044;                         // note: this shifts the epoch back to astronomical year -4800 instead of the start of the Christian era in year AD 1 of the proleptic Gregorian calendar
                long g = j / 146097; long dg = j % 146097;
                long c = ((dg / 36524 + 1) * 3) / 4; long dc = dg - c * 36524;
                long b = dc / 1461; long db = dc % 1461;
                long a = ((db / 365 + 1) * 3) / 4; long da = db - a * 365;
                long y = g * 400 + c * 100 + b * 4 + a;
                long m = (da * 5 + 308) / 153 - 2;          // note: this is the integer number of full months elapsed since the last March 1 at 00:00 UTC;
                long d = da - (m + 4) * 153 / 5 + 122;      // note: this is the number of days elapsed since day 1 of the month at 00:00 UTC, including fractions of one day;
                long Y = y - 4800 + (m + 2) / 12; long M = (m + 2) % 12 + 1; long D = d + 1; // note: astronomical Gregorian date (Y, M, D)

                year = Y - 1900;     // note: unix convention
                mon  = (int)M - 1;        // note: unix convention
                mday = (int)D;

                // To calculate DOY let's use da - it holds number of days in the Julian year (that begins on March 1).
                long N = da > 305 ? da - 305 : da + ((Y % 4 == 0 && Y % 100 != 0) || (Y % 400 == 0) ? 61 : 60);

                yday = (int)N;

                // Day of week calculation. Software implementation of Zeller's congruence. See http://en.wikipedia.org/wiki/Zeller%27s_congruence
                if (M < 3) {M = M + 12; Y = Y - 1;}
                long W = (D + (((M + 1) * 26) / 10) + Y + (Y / 4) + 6 * (Y / 100) + (Y / 400)) % 7;
                // Zeller's algorithm returns(0 = Saturday, 1 = Sunday, 2 = Monday, ...). We need (0 = Sunday, 1 = Monday, ...).
                W = (W + 6) % 7;
		...
		/// Leapseconds.
		/// </summary>
		private long[] leapSecs;

        /// <summary>
        /// Leapsecond corrections.
        /// </summary>
        private int[] leapCorrects;
        ...
        /// <summary>
        /// Gregorian epoch starting on March 1, -4800 (i.e. March 1, 4801 BC in the proleptic Gregorian Calendar)
        /// </summary>
        public const long MinClock = -213635404800;

		/// <summary>
		/// Biggest value of 64-bit unix clock.
		/// </summary>
        public const long MaxClock = long.MaxValue;

        /// <summary>
        /// Value of 64-bit unix clock corresponding to DateTime.MinValue;
        /// </summary>
        public const long MinDotNetClock = -62167219200L;
        ...
        public ZoneInfo(string name, string dir)
        {
            ...
            {
                // read 1st header
                byte[] tzMagic = reader.ReadBytes(4);
                string TZif = Encoding.ASCII.GetString(tzMagic);
                if (TZif != "TZif")
                {
                    throw new System.ArgumentException(string.Format("Invalid head magic signature. Expected 'TZif', got '{0}').", tzMagic));
                }
                reader.Seek(16, SeekOrigin.Current); // skip reserved 16 bytes
                int isGmtCount = reader.ReadInt32();
                int isStdCount = reader.ReadInt32();
                ...
                reader.Seek(toSkip, SeekOrigin.Current); // skip 32-bit time data

                // read 2nd header
                tzMagic = reader.ReadBytes(4);
                TZif = Encoding.ASCII.GetString(tzMagic);
                if (TZif != "TZif")
                {
                    throw new System.ArgumentException(string.Format("Invalid head magic signature. Expected 'TZif', got '{0}').", tzMagic));
                }
                ...
                reader.Seek(16, SeekOrigin.Current); // skip reserved 16 bytes
                isGmtCount = reader.ReadInt32();
                isStdCount = reader.ReadInt32();
                leapCount = reader.ReadInt32();
                timeCount = reader.ReadInt32();
                ruleCount = reader.ReadInt32();
                charCount = reader.ReadInt32();

                // load transition data
                transTimes = new long[timeCount];
                for (int i = 0; i < timeCount; i++)
                {
                    transTimes[i] = reader.ReadInt64();
                }
                ...
                leapSecs = new long[leapCount];
                leapCorrects = new int[leapCount];

                for (int i = 0; i < leapCount; i++)
                {
                    leapSecs[i] = reader.ReadInt64();
                    leapCorrects[i] = reader.ReadInt32();
                }
            }
            finally
            {
		...
		internal int ApplyRule(long clock, Rule rule, Time t) 
		{
			...
			for (int i = leapSecs.Length; (i -= 1) >= 0;) 
			{
				long time = leapSecs[i];
				int correction = leapCorrects[i];
				if (clock >= time) 
				{
					if (clock == time) 
					{
                        hit = ((i == 0 && correction > 0) || correction > leapCorrects[i - 1]);
					}
					offset -= correction;
					break;
				}
		...
		private long GetClock(Time time, bool local) 
		{
			...
			for (;;) 
			{
				...
				long direction = t.CompareTo(time);
				if (direction == 0) 
				{
					time.wday	= t.wday;
					time.yday	= t.yday;
		...
		public long[] GetTransitionClocks(int year) 
		{
			...
			{
				long clockYear = GetUtcTime(clock).year;
		...
		{
			return rules[transTypes[Array.IndexOf(transTimes, transitionClock)]];
		}
		...
		public long[] AllLeapSecondCorrectionClocks
		{
			get 
			{
				return (long[])new ArrayList(leapSecs).ToArray(typeof(long));
			}
		}
		...
		{
            for (int i = 0; i < leapSecs.Length; i++)
            {
                if (leapSecs[i] == correctionClock)
                {
                    return leapCorrects[i];
                }
            }
			return 0;
		}
		...
		{
			return GetRule(GetClockFromLocalTime(localTime)).IsDST;
		}
		...
		/// <returns>Seconds since the epoch.</returns>
		public long GetClockFromLocalTime(Time localTime) 
		{
		...
		{
			long clock = GetClockFromLocalTime(localTime);
