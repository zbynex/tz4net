...
namespace TZ4Net
{
	...
	public class OlsonTimeZone : System.TimeZone
	{
		...
		/// <summary>
		/// Min. value of time as defined by .NET. No longer limited to 13-12-1901 20:45:52 by 32-bit unix clock.
		/// </summary>
        public static readonly DateTime MinTime = DateTime.MinValue;

		/// <summary>
        /// Max. value of time as defined by .NET. No longer limited to 19-01-2038 03:14:07 by 32-bit unix clock.
		/// </summary>
		...
		/// </summary>
        private static readonly long futureTransitionsStartClock = 946684800; // Sat, 1 Jan 2000 00:00:00 UTC

		/// <summary>
		/// Max number of future transitions to be taken into consideration.
		/// </summary>
        private static readonly int maxFutureTransitions = 37; // from year 2000 to 2037
		...
		public override DateTime ToUniversalTime(DateTime time)
		{
			...
			return zoneInfo.GetUtcTime(zoneInfo.GetClockFromLocalTime(new ZoneInfo.Time(time))).DateTime;
		}
        ...
        public TimeCheckResult CheckLocalTime(DateTime time)
        {
            ...
            {
                if (time >= AllTimeChanges[i].Start && (time < AllTimeChanges[i].End || AllTimeChanges[i].End == MaxTime))
                {
            ...
            if (time >= currentChange.Start)
            {
                if (time < SafeAdd(currentChange.Start, currentChange.Delta))
                {
            ...
            if (nextChange != null)
            {
                if (time <= currentChange.End)
                {
                    if (time >= SafeAdd(currentChange.End, nextChange.Delta))
                    {
            ...
            else
            {
                ...
                // Note: This is boundary condition.
                // We assume there will be no transition defined exactly at DateTime.MaxValue, otherwise additional check against its offset must be done.
                // Theoretically this is possible as max. value of 64-bit Unix time is greater than DateTime.MaxValue i.e 23:12:9999 23:59:59
            }
		...
		private static double CalculateRuleCoverage(long startClock, long endClock) 
		{
			...
			double x1 = - (double)ZoneInfo.MinClock + startClock;
			double x2 = - (double)ZoneInfo.MinClock + endClock;
		...
		private static RankValue AddRuleToMap(Hashtable ruleMap, string name, ZoneInfo.Rule rule, long startClock, long endClock) 
		{
			...
			if (startClock >= futureTransitionsStartClock) 
			{
			...
			foreach (string name in olsonNames) 
			{
				...
				for (int i = 0; i <= imax; i++) 
				{
					...
					if (includeDST || !rule.IsDST) 
					{
						...
						rankValue.IsLastTransition = (startClock <= futureTransitionsStartClock);
					}
				}
        ...
        /// <summary>
        /// Adds safely TimeSpan value to given DateTime value.
        /// </summary>
        /// <param name="dt">Value to be added to.</param>
        /// <param name="ts">Value to add.</param>
        /// <returns></returns>
        public static DateTime SafeAdd(DateTime dt, TimeSpan ts)
        {
            if (ts > TimeSpan.Zero)
            {
                return dt.Ticks + ts.Ticks > DateTime.MaxValue.Ticks ? DateTime.MaxValue : dt + ts;
            }
            else
            {
                return dt.Ticks + ts.Ticks < DateTime.MinValue.Ticks ? DateTime.MinValue : dt + ts;
            }
        }
